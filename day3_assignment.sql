...
INSERT INTO EMP_TEST(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL,DEPTNO)
VALUES (7203,'TEST_USER3','ANALYST',7201,'2016-04-11',3400, 50);

INSERT INTO EMP_TEST(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL,DEPTNO)
VALUES (7204,'TEST_USER4','SALESMAN',7201,'2016-05-31',2700, 60);

INSERT INTO EMP_TEST(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL,DEPTNO)
VALUES (7205,'TEST_USER5','CLERK',7201,'2016-07-20',2600, 50);

INSERT INTO EMP_TEST(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, DEPTNO)
VALUES (7206,'TEST_USER6','CLERK',7201,'2016-09-08',2600, 50);
...
--3. EMP_TEST 테이블에서 DEPTNO = 50인 직원의 평균 월급인 직원보다 큰 경우, DEPTNO를 70으로  값을 갱신 하세요
--문제 오류... 해당 테이블에  DEPTNO = 50인 사람 없음...그냥 급여로만 진행해봄
UPDATE EMP_TEST
 SET DEPTNO = 70
 WHERE DEPTNO =50 
...
--4. EMP_TEST 테이블에서 DEPTNO =60인 직원 중에서 입사일이 가장 빠른 직원 이후에 입사한 직원의 DEPTNO를 80으로 변경하고  월급을  20%인상한 수치로 갱신하시오.
...
--2. EMP_USER 테이블에 DATE,  퇴사일 컬럼(RESIGN_DATE)를 추가해보세요.
...
--4. SUR_NAME 문자열 10자리로 변경
ALTER TABLE EMP_USER
MODIFY SUR_NAME VARCHAR(10);
...
--P249
--1. EMP 테이블과 동일한 구조의 EMP_IDX테이블을 생성한 후 EMPNO컬럼에 인덱스를 지정하고 인덱스명은 EMP_EMPNO_IDX로 지정하세요.
...
SELECT *
 FROM USER_INDEXES
  WHERE TABLE_NAME IN ('EMP_EMPNO_IDX');

--3. CREATE OR REPLACE VIEW 명령어를 사용하여 EMP_IDX 테이블에서 SAL이 20,000이상인 직원을 추출하고 COMM이 있는 경우 Y, 없거나 NULL인 경우 N으로 표기하세요.
-- 20,000이상 없음... 2000이상!!!!

CREATE VIEW VW_EMP
AS (SELECT EMPNO, ENAME, JOB, DEPTNO, SAL, NVL2(COMM, 'Y', 'N') AS COMM
